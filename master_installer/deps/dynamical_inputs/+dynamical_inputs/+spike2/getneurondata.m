...
if wasOpened
  cleanupObj = onCleanup(@() dynamical_inputs.spike2.closefile(fh));
end

valid_channels = dynamical_inputs.spike2.getChannelsOfType(fh, [5,6]);
...
neuronData = arrayfun(@(cid) dynamical_inputs.spike2.getneurondatachannel(input1, cid), channels);
